...
def printAstarNodeTable(graph, current):  # this is just to make it easier to see what is going on
    ...
    for node in graph:
        print(str(node) + "       " + str(node.visited).rjust(5) + "     " + str(node.shortestDistanceFromStart).rjust(
            6) + "    " + str(node.heuristicWeight).rjust(6) + "       " + str(node.fullPathEstimate).rjust(
            6) + "     " + str(node.previous).rjust(6), end="")
        if current == node:
            print("<-- Current")
        else:
            print()
...
def dijkstra(graph, start, end):
    ...
    while current != end:
        for linkedNode in current.links:
            checksmade += 1
            if linkedNode.shortestDistanceFromStart == None or current.shortestDistanceFromStart + current.links[
                linkedNode] < linkedNode.shortestDistanceFromStart:
                linkedNode.shortestDistanceFromStart = current.shortestDistanceFromStart + current.links[
                    linkedNode]  # update the distances
        ...
        for thisnode in graph:
            checksmade += 1
            if thisnode.visited == False and thisnode.shortestDistanceFromStart != None and (
                    shortest == None or thisnode.shortestDistanceFromStart < shortest):
    ...
    while not end.visited:
        for linkedNode in current.links:
            if linkedNode.visited == False:
                checksmade += 1
                if linkedNode.fullPathEstimate == None or current.shortestDistanceFromStart + current.links[
                    linkedNode] + linkedNode.heuristicWeight < linkedNode.fullPathEstimate:
                    linkedNode.fullPathEstimate = current.shortestDistanceFromStart + current.links[
                        linkedNode] + linkedNode.heuristicWeight  # update the distances
        ...
        for thisnode in graph:
            checksmade += 1
            if thisnode.visited == False and thisnode.fullPathEstimate != None and (
                    shortest == None or thisnode.fullPathEstimate < shortest):
